/**
 * ElectricityBillInputFields - 電費帳單輸入欄位組件
 *
 * 用於 Type 2 架構的「使用數據」編輯卡右側表單（單筆編輯模式）
 */

import { useRef } from 'react'
import { BillEditingGroup, ElectricityBillRecord, ElectricityMeter } from '../../../types/electricityTypes'
import { rocToISO, isoToROC } from '../../../utils/bill/dateCalculations'

interface ElectricityBillInputFieldsProps {
  /** 當前編輯群組 */
  currentGroup: BillEditingGroup
  /** 更新帳單欄位 */
  onUpdate: (id: string, field: keyof ElectricityBillRecord, value: any) => void
  /** 刪除帳單 */
...
export function ElectricityBillInputFields({
  currentGroup,
  onUpdate,
  meters,
  isReadOnly
}: ElectricityBillInputFieldsProps) {
  // 單筆模式：只顯示第一筆記錄
