/**
 * useOtherEnergySourcesSpecManager - 其他使用能源規格管理 Hook
 *
 * 功能：
 * - 管理規格列表狀態
 * - 管理當前編輯規格
...
export interface OtherEnergySourcesSpec {
  id: string
  name: string
...
const createEmptySpec = (): OtherEnergySourcesSpec => ({
  id: generateRecordId(),
  name: '',
  memoryFiles: [],
  evidenceFiles: []
...
export function useOtherEnergySourcesSpecManager() {
  const [savedSpecs, setSavedSpecs] = useState<OtherEnergySourcesSpec[]>([])
  const [currentEditingSpec, setCurrentEditingSpec] = useState<OtherEnergySourcesSpec>(createEmptySpec())
  const [editingSpecId, setEditingSpecId] = useState<string | null>(null)

  // 更新當前編輯規格的欄位
  const updateCurrentSpec = (field: keyof OtherEnergySourcesSpec, value: any) => {
  ...
  // 刪除規格
  // ⚠️ 注意：外部調用前需要檢查是否有使用記錄（在 OtherEnergySourcesPage 處理）
