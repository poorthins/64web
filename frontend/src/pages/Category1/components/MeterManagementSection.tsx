/**
 * MeterManagementSection - 天然氣錶號管理區塊
 *
 * 包含：
 * - 規格設定標題
 * - 錶號輸入框 + 新增按鈕
...
import { NaturalGasMeter, NaturalGasBillRecord } from '../../../types/naturalGasTypes'

interface MeterManagementSectionProps {
  /** 天然氣錶清單 */
  meters: NaturalGasMeter[]
  /** 已保存的帳單群組（用於檢查錶號是否被使用） */
...
export function MeterManagementSection({
  meters,
  savedGroups,
  newMeterInput,
      ...
      <div style={{ marginTop: '34px' }} className="flex justify-center">
        <div
          style={{
            ...
            height: '176px',
            borderRadius: '37px',
            background: 'linear-gradient(0deg, #49A1C7 0%, #49A1C7 100%)',
            position: 'relative'
          }}
        >
          {/* 表號清單 Label */}
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          ...
                          const inputElement = document.querySelector('input[placeholder="請輸入天然氣錶號碼"]') as HTMLInputElement
                          if (inputElement) {
                            inputElement.focus()
                      ...
                      <button
                        ...
                        className="hover:opacity-70 transition-opacity disabled:opacity-30 disabled:cursor-not-allowed"
                        title={isUsed ? '此錶號正在被使用，無法刪除' : '刪除錶號'}
                      >
