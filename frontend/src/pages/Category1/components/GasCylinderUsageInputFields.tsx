/**
 * GasCylinderUsageInputFields - 氣體鋼瓶使用數據輸入欄位組件
 *
 * 包含：
 * - 左側：上傳框（購買單據）
 * - 右側：表格式輸入（綠色表頭 + 白色輸入行）
...
interface GasCylinderUsageInputFieldsProps {
...
export function GasCylinderUsageInputFields({
  currentGroup,
  onUpdate,
  onUpdateSpecId,
  onDelete,
  ...
  thumbnails,
  onImageClick
}: GasCylinderUsageInputFieldsProps) {
              ...
              {currentGroup.records.map((record) => (
                <div key={record.id} className="flex items-center" style={{ gap: '27px' }}>
                  ...
                  <div className="relative" style={{ width: '199px' }}>
                    <input
                      id={`gas-cylinder-date-input-${record.id}`}
                      type="date"
                      value={record.date}
                    ...
                    <div
                      ...
                      onClick={() => {
                        const input = document.getElementById(`gas-cylinder-date-input-${record.id}`) as HTMLInputElement
