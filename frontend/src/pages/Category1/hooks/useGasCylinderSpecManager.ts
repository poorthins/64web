/**
 * useGasCylinderSpecManager - 氣體鋼瓶規格管理 Hook
 *
 * 功能：
 * - 管理規格列表狀態
 * - 管理當前編輯規格
...
export interface GasCylinderSpec {
  id: string
  name: string
...
const createEmptySpec = (): GasCylinderSpec => ({
  id: generateRecordId(),
  name: '',
  memoryFiles: [],
  evidenceFiles: []
...
export function useGasCylinderSpecManager() {
  const [savedSpecs, setSavedSpecs] = useState<GasCylinderSpec[]>([])
  const [currentEditingSpec, setCurrentEditingSpec] = useState<GasCylinderSpec>(createEmptySpec())
  const [editingSpecId, setEditingSpecId] = useState<string | null>(null)

  // 更新當前編輯規格的欄位
  const updateCurrentSpec = (field: keyof GasCylinderSpec, value: any) => {
  ...
  // 刪除規格
  // ⚠️ 注意：外部調用前需要檢查是否有使用記錄（在 GasCylinderPage 處理）
