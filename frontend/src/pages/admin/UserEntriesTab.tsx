...
import { ArrowLeft, Calendar, Filter, Download, CheckCircle, XCircle, AlertTriangle } from 'lucide-react'
...
const UserEntriesTab: React.FC<UserEntriesTabProps> = ({ userId, userName, onBack }) => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/users/${userId}/entries?${params}`, {
        headers: {
  ...
  const handleReview = async (entryId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/entries/${entryId}/review`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json'
