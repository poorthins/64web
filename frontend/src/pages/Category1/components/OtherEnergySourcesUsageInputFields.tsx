/**
 * OtherEnergySourcesUsageInputFields - 其他使用能源使用數據輸入欄位組件
 *
 * 包含：
 * - 左側：上傳框（購買單據）
 * - 右側：表格式輸入（深藍色表頭 + 白色輸入行）
 *   - 購買日期 + 品項選單 + 數量(KG)
 */
...
interface OtherEnergySourcesUsageInputFieldsProps {
...
export function OtherEnergySourcesUsageInputFields({
  currentGroup,
  onUpdate,
  onUpdateSpecId,
  onDelete,
  ...
  thumbnails,
  onImageClick
}: OtherEnergySourcesUsageInputFieldsProps) {
        ...
        <div style={{ width: '772px', display: 'flex', flexDirection: 'column', marginTop: '89px' }}>
          ...
          <div className="flex items-center" style={{ backgroundColor: iconColor, height: '58px', paddingLeft: '43px', paddingRight: '16px', borderTopLeftRadius: '30px', borderTopRightRadius: '30px' }}>
            ...
            <div style={{ width: '230px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span className="text-white text-[20px] font-medium">購買數量 (KG)</span>
            </div>
              ...
              {currentGroup.records.map((record) => (
                <div key={record.id} className="flex items-center" style={{ gap: '27px' }}>
                  ...
                  <div className="relative" style={{ width: '199px' }}>
                    <input
                      id={`other-energy-date-input-${record.id}`}
                      type="date"
                      value={record.date}
                    ...
                    <div
                      ...
                      onClick={() => {
                        const input = document.getElementById(`other-energy-date-input-${record.id}`) as HTMLInputElement
