...
**核心功能**:
- 📊 **狀態分類顯示**：4個統計卡片（已提交、待審核、已通過、已退回）
- 🔍 **智慧篩選**：依據用戶、能源類別、時間排序篩選記錄
- 📋 **列表式管理**：清晰顯示填報記錄，支援點擊查看詳情

#### 3.2 審核操作功能 🆕
**待審核記錄**:
- ✅ **通過按鈕**：一鍵通過審核，記錄狀態變更為「已通過」
- ❌ **退回按鈕**：彈出退回原因輸入框，支援詳細說明
- 📝 **退回原因管理**：必填驗證，支援多行文字輸入
...
- **原因展示**：清楚顯示退回原因和退回時間
- **操作記錄**：記錄退回者和操作時間
- **視覺區別**：紅色背景區塊，明確標示退回狀態

#### 3.3 審核工作流程
```
用戶提交填報 → [已提交] → 管理員審核 → [待審核]
                                    ↓
                           [通過] 或 [退回+原因]
                                    ↓
                          [已通過] 或 [已退回]
...
**審核操作場景**:
1. **管理員進入填報管理** → 點擊「待審核」卡片
2. **查看待審核記錄** → 每筆記錄顯示 [通過] [退回] 按鈕
3. **點擊退回** → 彈出原因輸入框 → 填寫詳細原因 → 確認退回
4. **查看退回記錄** → 點擊「已退回」卡片 → 顯示退回原因和時間
