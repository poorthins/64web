...
const NaturalGasPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 提交資料除錯 (可在需要時啟用)
      // console.log('🔍 [NaturalGas] 準備提交的資料:', entryInput)
      ...
      setToast({
        message: `提交成功！年度總使用量：${totalUsage.toFixed(2)} m³`,
        type: 'success'
      })
      ...
      // 詳細的錯誤記錄
      console.error('❌ [NaturalGas] 提交失敗，完整錯誤訊息:', error)
      console.error('❌ [NaturalGas] 錯誤類型:', error?.constructor?.name)
      console.error('❌ [NaturalGas] 錯誤詳情:', (error as any)?.details)
      console.error('❌ [NaturalGas] 錯誤提示:', (error as any)?.hint)
      console.error('❌ [NaturalGas] 錯誤代碼:', (error as any)?.code)
      console.error('❌ [NaturalGas] 錯誤堆疊:', (error as any)?.stack)

      if (error && typeof error === 'object') {
        console.error('❌ [NaturalGas] 錯誤物件所有屬性:', Object.keys(error))
        console.error('❌ [NaturalGas] 完整錯誤物件:', error)
      }
      ...
      try {
        ...
        if (existingEntry && existingEntry.status !== 'draft') {
          ...
          try {
            ...
            console.error('載入熱值佐證文件失敗:', error)
          }
        }
            ...
            {bills.map((bill, index) => (
              <div key={bill.id}>
                <div className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-lg">
                  ...
                  {/* 計費度數 */}
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600">計費度數</span>
                    <input
                      type="number"
                      placeholder="0"
                      min="0"
