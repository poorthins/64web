/**
 * useAcetyleneSpecManager - 乙炔規格管理 Hook
 *
 * 功能：
 * - 管理規格列表狀態
 * - 管理當前編輯規格
...
export interface AcetyleneSpec {
  id: string
  name: string
...
const createEmptySpec = (): AcetyleneSpec => ({
  id: generateRecordId(),
  name: '',
  memoryFiles: [],
  evidenceFiles: []
...
export function useAcetyleneSpecManager() {
  const [savedSpecs, setSavedSpecs] = useState<AcetyleneSpec[]>([])
  const [currentEditingSpec, setCurrentEditingSpec] = useState<AcetyleneSpec>(createEmptySpec())
  const [editingSpecId, setEditingSpecId] = useState<string | null>(null)

  // 更新當前編輯規格的欄位
  const updateCurrentSpec = (field: keyof AcetyleneSpec, value: any) => {
  ...
  // 刪除規格
  // ⚠️ 注意：外部調用前需要檢查是否有使用記錄（在 AcetylenePage 處理）
