/**
 * AcetyleneUsageInputFields - 乙炔使用數據輸入欄位組件
 *
 * 包含：
 * - 左側：上傳框（購買單據）
 * - 右側：表格式輸入（藍色表頭 + 白色輸入行）
...
interface AcetyleneUsageInputFieldsProps {
...
export function AcetyleneUsageInputFields({
  currentGroup,
  onUpdate,
  onUpdateSpecId,
  onDelete,
  ...
  thumbnails,
  onImageClick
}: AcetyleneUsageInputFieldsProps) {
              ...
              {currentGroup.records.map((record) => (
                <div key={record.id} className="flex items-center" style={{ gap: '27px' }}>
                  ...
                  <div className="relative" style={{ width: '199px' }}>
                    <input
                      id={`acetylene-date-input-${record.id}`}
                      type="date"
                      value={record.date}
                    ...
                    <div
                      ...
                      onClick={() => {
                        const input = document.getElementById(`acetylene-date-input-${record.id}`) as HTMLInputElement
