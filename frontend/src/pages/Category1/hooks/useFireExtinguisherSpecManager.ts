/**
 * useFireExtinguisherSpecManager - 滅火器規格管理 Hook
 *
 * 功能：
 * - 管理規格列表狀態
 * - 管理當前編輯規格
...
export interface FireExtinguisherSpec {
  id: string
  name: string            // 品項清單（格式：設備類型_單支藥劑量 (KG/支)）
  memoryFiles: MemoryFile[]
  evidenceFiles?: EvidenceFile[]
}

const createEmptySpec = (): FireExtinguisherSpec => ({
  id: generateRecordId(),
  name: '',
  memoryFiles: [],
  evidenceFiles: []
...
export function useFireExtinguisherSpecManager() {
  const [savedSpecs, setSavedSpecs] = useState<FireExtinguisherSpec[]>([])
  const [currentEditingSpec, setCurrentEditingSpec] = useState<FireExtinguisherSpec>(createEmptySpec())
  const [editingSpecId, setEditingSpecId] = useState<string | null>(null)

  // 更新當前編輯規格的欄位
  const updateCurrentSpec = (field: keyof FireExtinguisherSpec, value: any) => {
  ...
  // 刪除規格
  // ⚠️ 注意：外部調用前需要檢查是否有使用記錄（在 FireExtinguisherPage 處理）
