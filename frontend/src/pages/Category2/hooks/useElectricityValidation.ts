/**
 * useElectricityValidation - 電費頁面驗證邏輯 Hook
 *
 * 負責：
 * - 帳單記錄欄位驗證
 * - 群組完整性驗證
...
export interface GroupValidationResult {
  isValid: boolean
  validRecords: ElectricityBillRecord[]
  error?: string
}
...
export function useElectricityValidation() {
   ...
   * 驗證單筆帳單記錄
   */
  const validateBillRecord = useCallback((bill: ElectricityBillRecord): ValidationResult => {
    // 檢查必填欄位
    if (!bill.meterId) {
    ...
    if (!bill.billingUnits || bill.billingUnits <= 0) {
      return { isValid: false, error: '請填寫有效的用電度數' }
    }
   ...
   * 驗證群組（過濾出有效記錄）
   */
  const validateGroup = useCallback((records: ElectricityBillRecord[]): GroupValidationResult => {
  ...
  const validateMeter = useCallback((meterNumber: string): ValidationResult => {
    ...
    if (trimmed.length === 0) {
      return { isValid: false, error: '表號不能為空' }
    }
  ...
  const checkDuplicateMeter = useCallback((
    meterNumber: string,
    existingMeters: ElectricityMeter[]
  ): ValidationResult => {
    const trimmed = meterNumber.trim()
    const isDuplicate = existingMeters.some(m => m.meterNumber === trimmed)

    if (isDuplicate) {
      return { isValid: false, error: '此表號已存在' }
    }
  ...
  const validateBeforeSubmit = useCallback((
    ...
    // 驗證：至少要有一個表號
    if (meters.length === 0) {
      return { isValid: false, error: '請至少新增一個電表表號' }
    }
