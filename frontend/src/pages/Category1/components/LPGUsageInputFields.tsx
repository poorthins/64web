/**
 * LPGUsageInputFields - LPG 使用數據輸入欄位組件
 *
 * 包含：
 * - 左側：上傳框（購買單據）
 * - 右側：表格式輸入（綠色表頭 + 白色輸入行）
...
interface LPGUsageInputFieldsProps {
  ...
  onRemoveEvidence: () => void  // ⭐ 新增：清除佐證資料
  specs: LPGSpec[]
  isReadOnly: boolean
...
export function LPGUsageInputFields({
  currentGroup,
  onUpdate,
  onUpdateSpecId,
  onDelete,
  ...
  return (
    <div style={{ marginTop: '39px', marginLeft: '367px' }}>
      <div style={STYLES.container} className="flex gap-[32px]">
        {/* 左側：上傳框 */}
        <div style={{ flexShrink: 0 }}>
          <FileDropzone
            ...
            onRemove={onRemoveEvidence}  // ⭐ 清除所有佐證（memoryFiles + evidenceFiles）
        ...
        {/* 右側：表格式輸入區 */}
        <div style={{ width: '772px', display: 'flex', flexDirection: 'column' }}>
          {/* 表頭 - 綠色區域 */}
          <div className="flex items-center" style={{ backgroundColor: '#2DB14C', height: '58px', paddingLeft: '43px', paddingRight: '16px', borderTopLeftRadius: '30px', borderTopRightRadius: '30px' }}>
            ...
            <div style={{ width: '230px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span className="text-white text-[20px] font-medium">購買數量 (桶)</span>
            </div>
              ...
              {currentGroup.records.map((record) => (
                <div key={record.id} className="flex items-center" style={{ gap: '27px' }}>
                  ...
                  <div className="relative" style={{ width: '199px' }}>
                    <input
                      id={`lpg-date-input-${record.id}`}
                      type="date"
                      value={record.date}
                      onChange={(e) => onUpdate(record.id, 'date', e.target.value)}
                      disabled={isReadOnly}
                      className="rounded-[5px] focus:outline-none focus:ring-2 focus:ring-green-500 [&::-webkit-calendar-picker-indicator]:hidden"
                      style={{
                        ...STYLES.field,
                        width: '199px',
                        paddingRight: '48px',
                    ...
                    <div
                      ...
                      onClick={() => {
                        const input = document.getElementById(`lpg-date-input-${record.id}`) as HTMLInputElement
                  ...
                  <select
                    ...
                    className="rounded-[5px] focus:outline-none focus:ring-2 focus:ring-green-500"
                    style={{
                      ...STYLES.field,
                      width: '190px',
          ...
          <button
            ...
            style={{
              ...
              height: '45.984px',
              borderRadius: '7px',
              background: isReadOnly ? '#9CA3AF' : '#2DB14C',
              color: '#FFF',
              textAlign: 'center',
