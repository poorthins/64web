...
export default function WD40Page() {
  ...
  const updateMonthData = (index: number, field: keyof MonthData, value: any) => {
    setMonthlyData(prev => {
      ...
      if (field === 'quantity') {
        newData[index].totalUsage = newData[index].quantity * unitCapacity;
      }
  ...
  const handleCalculate = () => {
    ...
    alert(`計算完成！年度總使用量：${getTotalUsage().toFixed(2)} ML`);
  };

  return (
    <div className="space-y-8">
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-6">
        <h2 className="text-xl font-semibold text-brand-800 mb-3">WD-40 碳排放計算</h2>
        <p className="text-gray-600">請上傳MSDS文件並填入各月份使用數據進行碳排放計算</p>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-6">MSDS 安全資料表與基本參數</h3>
        
        <div className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">MSDS安全資料表</label>
            <FileUpload
              onFileSelect={setMsdsFile}
              accept=".pdf,application/pdf"
              currentFile={msdsFile}
              placeholder="請上傳MSDS安全資料表 (PDF格式)"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 hover:border-brand-300 transition-colors duration-200"
                placeholder="請輸入單位容量"
              />
            </div>
          ...
          <table className="w-full border-collapse bg-white">
            <thead>
              <tr className="bg-gradient-to-r from-brand-500 to-brand-600">
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-white border-r border-brand-400/30">使用數量(瓶)</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-white border-r border-brand-400/30">總使用量(ML)</th>
            ...
            <tbody>
              {monthlyData.map((data, index) => (
                <tr key={index} className="hover:bg-brand-50 transition-colors duration-200 border-b border-gray-100">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      <div className="px-3 py-2 bg-brand-100 text-brand-700 font-semibold text-sm rounded-lg min-w-[80px] text-center">
                        {data.totalUsage.toFixed(2)}
                      </div>
              ...
              <tr className="bg-gradient-to-r from-brand-100 to-brand-50 border-t-2 border-brand-300">
                ...
                <td className="px-6 py-5">
                  <div className="px-3 py-2 bg-brand-200 text-brand-800 font-bold text-sm rounded-lg text-center">
                    {monthlyData.reduce((sum, data) => sum + data.quantity, 0).toFixed(1)}
                  </div>
                </td>
                <td className="px-6 py-5">
