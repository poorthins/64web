/**
 * useWD40SpecManager - WD-40 規格管理 Hook
 *
 * 功能：
 * - 管理規格列表狀態
 * - 管理當前編輯規格
...
const createEmptySpec = (): WD40Spec => ({
  id: generateRecordId(),
  name: '',
  memoryFiles: [],
  evidenceFiles: []
...
export function useWD40SpecManager() {
  const [savedSpecs, setSavedSpecs] = useState<WD40Spec[]>([])
  const [currentEditingSpec, setCurrentEditingSpec] = useState<WD40Spec>(createEmptySpec())
  const [editingSpecId, setEditingSpecId] = useState<string | null>(null)

  // 更新當前編輯規格的欄位
  const updateCurrentSpec = (field: keyof WD40Spec, value: any) => {
  ...
  // 刪除規格
  // ⚠️ 注意：外部調用前需要檢查是否有使用記錄（在 WD40Page 處理）
